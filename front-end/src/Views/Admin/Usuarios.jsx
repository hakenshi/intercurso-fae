...
export const Usuarios = () => {
    ...
    useEffect(() => {
        if (!usuarios) {
            axiosInstance.get("/usuarios-responsaveis")
                .then(response => {
    ...
    return (
        <>
            ...
            <div className="w-full h-[88vh] flex items-center flex-col">
                <h1 className="text-center p-5 text-3xl font-medium">Usuários Responsáveis</h1>
                <div className="flex flex-col">
                    <span className="flex justify-around p-5">
                        <button onClick={() => setIsAlertOpen(true)} className="w-fit p-3 btn-green text-sm ">Cadastrar Responsável</button>
                            ...
                            <tbody className="divide-y-2 divide-unifae-gray50-2">
                                {usuarios.map(response => (
                                    <tr key={response.usuario.id} className="text-center">
                                        <td className="p-5">{response.usuario.nome}</td>
                                        <td className="p-5">{response.usuario.email}</td>
                                        <td className="p-5">{response.curso.nome_curso}</td>
                                        <td className="p-5">{!response.usuario.telefone ? "Sem Telefone" : response.usuario.telefone}</td>
                                        <td className="p-5">{response.usuario.ra}</td>
                                        <td className="p-5">{response.usuario.tipo_usuario}</td>
                                        <td className="p-5 flex gap-5"><button onClick={() => handleEditModal(response)} className="btn-sm btn-edit">Editar</button> <button onClick={() => handleDelete(response.usuario.id)} className="btn-sm btn-delete">Excluir</button></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>)}
