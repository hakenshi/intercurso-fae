    ...
    <>
      ...
      {isOpen === 'editar' && (
        <Modal.Root isOpen onClose={handleCloseEditJogos} >
          <Modal.Form texto={"Editar Jogo"} onSubmit={(e) => handleSubmit(e, "patch", editJogos.jogo.id ?? null)}>
            <div className="flex flex-col justify-center p-2">
              <label>Primerio Time</label>
              <Search handleSelectUser={handleSelectTime1} url={"/search-times"} placeholder={"Insira o nome do time 1"} data={editJogos.times.time1.nome ?? ""} />
            </div>
            <div className="flex flex-col justify-center p-2">
              <label>Segundo Time</label>
              <Search handleSelectUser={handleSelectTime2} url={"/search-times"} placeholder={"Insira o nome do time 2"} data={editJogos.times.time2.nome ?? ""} />
            </div>
            <Input ref={dataRef} mask={"99/99/9999"} label={"Data do jogo"} name={"data"} className={"input-modal"} placholder={"Insira um data para o jogo"} value={editJogos.jogo.data_jogo ?? ""} />
            <Input ref={horaRef} mask={"99:99"} value={editJogos.jogo.hora_jogo ?? ""} label={"Hora do jogo"} name={"hora"} className={"input-modal"} placholder={"Insira uma hora para o jogo"} />
            <div className="flex flex-col justify-center p-2">
              <label htmlFor={"local"}>Local</label>
              <input defaultValue={editJogos.jogo.local ?? ""} ref={localRef} type="text" className={"input-modal"} name={"local"} placeholder={"Insira o local do jogo"} />
            </div>
            <div className="flex flex-col justify-center p-2">
              <label htmlFor="modalidade">Modalidade do jogo</label>
              <select ref={modalidadeRef} defaultValue={editJogos.modalidade.id ?? ""} className='input-modal bg-white' name="modalidade" id="modalidade">
                <option value="">Selecione uma modalidade</option>
                {modalidades.map(modalidade => (<option key={modalidade.id} value={modalidade.id}>{modalidade.nome}</option>))}
              </select>
            </div>
          </Modal.Form>
        </Modal.Root>
      ...
      {isOpen === 'cadastro' && (
        <Modal.Root isOpen onClose={() => setIsOpen(null)}>
          <Modal.Form texto={"Cadastrar Jogo"} onSubmit={(e) => handleSubmit(e, 'post')}>
            <div className="flex flex-col justify-center p-2">
              <label>Primerio Time</label>
              <Search handleSelectUser={handleSelectTime1} url={"/search-times"} placeholder={"Insira o nome do time 1"} />
            </div>
            <div className="flex flex-col justify-center p-2">
              <label>Segundo Time</label>
              <Search handleSelectUser={handleSelectTime2} url={"/search-times"} placeholder={"Insira o nome do time 2"} />
            </div>
            <Input ref={dataRef} mask={"99/99/9999"} label={"Data do jogo"} name={"data"} className={"input-modal"} placholder={"Insira um data para o jogo"} />
            <Input ref={horaRef} mask={"99:99"} label={"Hora do jogo"} name={"hora"} className={"input-modal"} placholder={"Insira uma hora para o jogo"} />
            <div className="flex flex-col justify-center p-2">
              <label htmlFor={"local"}>Local</label>
              <input ref={localRef} type="text" className={"input-modal"} name={"local"} placeholder={"Insira o local do jogo"} />
            </div>
            <div className="flex flex-col justify-center p-2">
              <label htmlFor="modalidade">Modalidade do jogo</label>
                ...
                {jogos && jogos.map((response, i) =>
                  <tr key={i} className='text-center'>
                    ...
                    <td className='p-5 flex gap-5'>
                      ...
                      <button onClick={() => handleConfirm(response.jogo.id)} className='btn-delete p-2'>Apagar</button>
                    </td>
                  </tr>
                )}
              </Table.Body>
            </Table.Root>
            <Paginate currentPage={currentPage} handlePageChange={handlePageChange} lastPage={lastPage} /> </>) : <p>Não já jogos cadastrados no sistema</p>}
        </Display.Main>
      </Display.Root>
    </>
  )
