    ...
    return (
        <>
            ...
            {loading ?
                ...
                </div> : times.length > 0 ? <div className="w-full flex items-center flex-col">
                        ...
                        <Table.Body>
                            {times
                                ...
                                .map((time, index) => <tr className='text-center' key={index}>
                                    ...
                                    <td className='p-5'>{time.informacoes.jogadores.map(({
                                                                                             status,
                                                                                             id_time,
                                                                                             id
                                                                                         }) => status === "0" ? (
                                        <div key={id_time} className='flex justify-center gap-4'>
                                            <button onClick={() => handleAceitarTime(id)}
                                                    className='p-2 h-7 w-7 md:w-10 md:h-10 text-xs md:text-base text-center btn-confirm'>
                                                <FontAwesomeIcon icon={faCheck}/></button>
                                            <button onClick={() => handleRejeitarTime(id)}
                                                    className='p-2 h-7 w-7 md:w-10 md:h-10 text-xs md:text-base text-center btn-delete'>
                                                <FontAwesomeIcon icon={faX}/></button>
                                        </div>
                                    ) : <div className='flex w-full justify-center gap-4' key={id_time}>
                                        <ButtonVerJogadores key={index}
                                                            onClick={() => handleJogadores(time.informacoes.jogadores)}/>
                                        <button onClick={() => handleSair(id, id_time)}
                                                className="btn-delete p-2 w-24">Sair
                                        </button>
                                    </div>)}
                                    </td>
                                </tr>)}
