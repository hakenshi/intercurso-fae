    ...
    <>
      <Modal.Root isOpen={editModal} hasButton={false}>
        ...
        <div className="relative md:flex md:justify-end md:items-center w-full md:h-8">
          <ButtonClose onClose={handleClose} className={"absolute text-unifae-white-1 md:text-unifae-gray-2"}/>
        </div>
        <Modal.Form onSubmit={handleSubmit}>
        <div className="flex justify-center items-center p-5 bg-gradient-to-tr from-unifae-green-3 via-unifae-green-2 to-unifae-green-4 h-1/3 rounded-t-lg">
          <div className='flex justify-center items-center flex-col gap-5'>
            <div className="relative z-0">
              <PreviewImage className={"rounded-full md:w-56 md:h-56 w-24 h-24 border-collapse border-4 border-unifae-green-4 shadow-xl object-cover"} fotoPerfil={user.foto_perfil} preview={profileImage} alt={user.nome} />
      ...
      <Container>
        <div className="flex flex-col p-5 bg-gradient-to-tr from-unifae-green-3 via-unifae-green-2 to-unifae-green-4 w-full md:w-[80vw] h-fit md:rounded-t-lg">
        <div className="flex w-full justify-end gap-5">
          ...
          <div className="flex justify-between gap-10 h-full w-full">
            <div className='flex justify-center items-center gap-10'>
                <ProfileImage className={"rounded-full lg:w-56 lg:h-56 w-32 h-32 border-collapse border-4 border-unifae-green-4 shadow-xl object-cover"} fotoPerfil={user.foto_perfil} />
              <div className='text-unifae-white-1 font-medium text-2xl md:text-3xl'>
                <p>{user.nome}</p>
                <i className='text-lg'>{user.tipo_usuario == 1 ? "Admin" : user.tipo_usuario == 2 ? "Responsável" : "Usuário"}</i>
            </div>
            </div>
              <div className="lg:flex lg:flex-col justify-center items-center text-white font-medium hidden"><img className='w-72' src={logoBranca} alt="unifae-logo" /> Intercurso
              </div>
          </div>
