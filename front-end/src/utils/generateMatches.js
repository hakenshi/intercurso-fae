...
export const generateMatches = (array) => {
    ...
    return {
        chaveChapeu: chaveChapeu.map((_, index) => {
            if (index % 2 === 0) 
                return {
                    [`jogos${index + 1}`]: {
        ...
        chavePrincipal: arrayFill(chavePrincipal, 2),
        quartas: arrayFill(quartas, 4),
        semifinais: arrayFill(semifinais, 8)
    };
};

const {chaveChapeu,chavePrincipal,quartas,semifinais} = generateMatches(teams)
